...
extension FriendsController : UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        locations.append(["latitude" : data[indexPath.row].latitude ?? 0.0 ,
                          "longitude" : data[indexPath.row].longitude ?? 0.0 ,
                          "icon" : data[indexPath.row].image ?? UIImage(named: "adduser")])
        return cell
    }
