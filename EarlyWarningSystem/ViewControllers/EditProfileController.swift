...
class EditProfileController: BaseViewController {
    ...
    @IBAction func saveBtn(_ sender: UIButton) {
        if !(txtFname.text?.isEmpty)! && !(txtLname.text?.isEmpty)! && !(txtAddress.text?.isEmpty)! && !(txtPhone.text?.isEmpty)! {
            SVProgressHUD.show()
            let user = UserModel(userID: nil, fname: txtFname.text!, lname: txtLname.text!, email: txtEmail.text!, address: txtAddress.text!, phone: txtPhone.text!, password: nil, latitude: nil, longitude: nil, image: nil)
