    ...
    func signUp(user : UserModel, completionHandler : @escaping completion) {
        ...
        Auth.auth().createUser(withEmail: user.email, password: user.password!) { (result, error1) in
            ...
            if error1 != nil {
                ...
                let userDict = ["FirstName":user.fname,"Lastname":user.lname,"Email":user.email,"Address":user.address,"Phone":user.phone, "Latitude": user.latitude, "Longitude": user.longitude] as [String : Any]
    ...
    func getUser(completionHandler : @escaping (UserModel?)->Void) {
        ...
        reference.child("User").child(user!.uid).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            self.getUserImg(id: user!.uid, completionHandler: { (data, error) in
                let use = UserModel(userID: user!.uid, fname: snap["FirstName"] as! String, lname: snap["Lastname"] as! String, email: user!.email!, address: snap["Address"] as! String , phone: snap["Phone"] as! String, password: nil, latitude: 37.43344, longitude: -121.3323, image: UIImage(data: data ?? Data()))
    ...
    func getUserByID(userID: String, completionHandler : @escaping (UserModel?)->Void) {
        ...
        reference.child("User").child(userID).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            self.getUserImg(id: userID, completionHandler: { (data, error) in
                let use = UserModel(userID: userID, fname: snap["FirstName"] as! String, lname: snap["Lastname"] as! String, email: snap["Email"] as! String, address: snap["Address"] as! String , phone: snap["Phone"] as! String, password: nil, latitude: (37.2131 as Double) , longitude: (-121.3232 as! Double),image: UIImage(data: data ?? Data()) )
                completionHandler(use)
            })
        })
    }
                ...
                for friend in friends {
                  ...
                  self.reference.child("User").child(friend.key).observeSingleEvent(of: .value) { (friendSnapShot) in
                        ...
                        var userModel = UserModel(userID: friend.key,
                                              ...
                                              password: nil,
                                              latitude: (singleFriend["latitude"] as? Double) ?? 0.0,
                                              longitude: (singleFriend["longitude"] as? Double) ?? 0.0,
                                              image: nil
                                              )
